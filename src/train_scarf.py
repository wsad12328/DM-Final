...
def main():
    ...
    if args.encoding == 'onehot':
        data_encoder = OneHotEncoderWrapper(cat_cols, num_cols)
    else:  # label encoding
        data_encoder = LabelEncoderWrapper(cat_cols, num_cols)
    
    data_encoder.fit(full_df)
    ...
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=40)
    ...
    encoder_model = Encoder(input_dim=input_dim, hidden_dim=256, dropout=0.2).to(device)
    proj_head = ProjectionHead(input_dim=256, projection_dim=256).to(device)
    optimizer = optim.AdamW(list(encoder_model.parameters()) + list(proj_head.parameters()), lr=args.lr)
    ...
    for epoch in range(args.epochs):
        ...
        for orig, aug, _ in tqdm(train_loader, desc=f"Epoch {epoch+1}/{args.epochs} [Train]"):
            ...
            h1 = encoder_model(orig)
            h2 = encoder_model(aug)
            z1 = proj_head(h1)
            z2 = proj_head(h2)
    ...
    torch.save(encoder_model.state_dict(), f"{save_dir}/scarf_encoder_{args.encoding}.pth")
    print(f"Pretrained encoder saved with {args.encoding} encoding.")
